...
public class Game{
	...
	//		if (!Grid.Map[(int)pa.Action.TargetPosition.X, (int)pa.Action.TargetPosition.Y].HasPellet)
		...
		foreach (Pac p in GetMyPacs().Where(p => !p.Action.HasAction).ToList())
		{
			...
			{
				List<Entity> AlreadyTargeted2 = GetMyPacs().Where(e => e.Id != p.Id).Select(e => e.Action.TargetEntity).ToList();

				var  target = Grid.Map.Cast<Cell>().Where(e => e.HasPellet).Select(e => e.Inside).Cast<Pellet>()
										.Except(AlreadyTargeted2).OrderBy(e => p.Distance(this, e.Position)).FirstOrDefault();
				if (target != null)
		...
		foreach (Pac p in GetMyPacs())
		{
			...
			if (!opponents.Any())
				continue;
			if(p.CanBeEatenBy(opponents.First()))
			{
